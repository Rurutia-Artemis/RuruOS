...
> 最后更新：2026-07-17 —— **v1.1.0 知识管线改版**：阅读器「☆ 珍藏」钮＋已读/珍藏自动浓缩归档（skill 聚合进 Skill档案/ 一份、笔记自带节选与 source_url、非珍藏原文进 .trash/）；「AI 浓缩/清杂」两钮退役换单颗「整理」（补漏/清尾/skill 档案去重/归位）；主屏两组玻璃托盘（日常管理/阅读与知识）＋组级隐身软糖把手统一调高；待读 15 篇翻页、知识沉淀五筛选（含珍藏书架）翻页；追剧墙默认「在追」；内滚动条全库糖霜统一；修阅读器已读/珍藏按钮 metadataCache 时序 bug（本地状态+就地重绘）。
> 上一版 2026-07-11 深夜 —— **🎉 v1.0 成品里程碑（用户验收满意，v8.0=1.0）**。顶栏（名牌/金句/统计/热力/天气/时钟黄历/方章）、追剧墙（搜剧/分季/置顶/轮播）、阅读器（外观面板/进度/备注）、AI 管线（搜剧补全/收录简介/搜评分）全部定稿。后续为增量开发。Mia 规则文档已交付桌面（含分季规则）。
...
3. ~~AI 清杂~~ 已于 v1.1.0 废除（入库把关已覆盖其场景）；浓缩改为已读/珍藏自动触发，「整理」按钮负责补漏
