...
- **Claude Code**：装好登录即可——插件里的 AI 按钮（浓缩/清杂/补剧集资料/搜评分）自动探测 claude CLI 激活；用户以后在库目录跑 `claude`，会自动读 `CLAUDE.md` / `AGENTS.md` 遵守写入规范
- **Codex**：用户在库目录跑 `codex` 即可，`AGENTS.md` 自动生效；插件 AI 按钮只认 claude CLI，不装 Claude Code 就告诉用户无视那几个按钮，同样的事对话里都能做
